...
const fs = require("fs") 
const { color, bgcolor } = require('./LENT/color')
const { help } = require('./LENT/help')
const kagApi = require('@kagchi/kag-api')
const { donasi } = require('./LENT/donasi')
const { fetchJson } = require('./LENT/fetcher')
const { recognize } = require('./LENT/ocr')
...
const welkom = JSON.parse(fs.readFileSync('./LENT/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./LENT/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./LENT/simi.json'))
...
const config = {
    A187: 'BOT LENT', // LENTBOT
    instagram: 'https://instagram.com/maslent11', //IG
    nomer: 'wa.me/628165466368', // NOMOR
    youtube: 'https://youtube.com/channel/UCGYLWtyT9IADYNUiK0uZiGg', // YT
    whatsapp: 'https://chat.whatsapp.com/DSSHmG2KjKJLoFp9B9mkVs', // WA
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Bot lent Scan the qr code '))
	})
              ...
              case 'abgjago':
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ 𝐏𝐄𝐒𝐀𝐍 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 MASLENT ❯\n\n${body.slice(4)}`})
						}
						reply('𝐬𝐮𝐜𝐜𝐬𝐬 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ BRODCAST MASLENT ❯\n\n${body.slice(4)}`)
						}
						reply('𝐬𝐮𝐜𝐜𝐬𝐬 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭')
					}
					break
